package com.syspeak.modules.unit.service;
...
import com.syspeak.modules.unit.entity.data.User;

@ContextConfiguration(locations = { "/applicationContext-core-test.xml" })
public class BaseEntityServiceImplTest extends SpringTxTestCase {

	private BaseEntityService<User, Long> service;
	private BaseEntityDao<User, Long> dao;
	...
	public void setUp() throws Exception {
		...
		dao = new BaseHibernateDao<User, Long>(User.class);
		((BaseHibernateDao<User, Long>) dao).setSessionFactory(sessionFactory);
		service = new BaseEntityServiceImpl<User, Long, BaseEntityDao<User, Long>>(dao);
	}
	...
	public void getAll() {
		...
		Page<User> page = service.getAll(pageRequest);
		...
		page = service.getAll(pageRequest);
	...
	public void findByHQL() {
		...
		PageRequest pageRequest = new PageRequest(1, 5);
		Page<User> page = service.findPage(pageRequest, "from User u where email like ?", "%springside.org.cn%");
		assertEquals(5, page.getResult().size());
		...
		//翻页
		pageRequest.setPageNo(2);
		page = service.findPage(pageRequest, "from User u where email like ?", "%springside.org.cn%");
		assertEquals(1, page.getResult().size());

		//命名参数版本
		Map<String, String> paraMap = Collections.singletonMap("email", "%springside.org.cn%");
		pageRequest = new PageRequest(1, 5);
		page = service.findPage(pageRequest, "from User u where email like :email", paraMap);
		assertEquals(5, page.getResult().size());
	...
	public void findByProperty() {
		List<User> users = service.findBy("loginName", "admin", PropertyFilter.MatchType.EQ);
		assertEquals(1, users.size());
		assertEquals("admin", users.get(0).getLoginName());

		users = service.findBy("email", "springside.org.cn", PropertyFilter.MatchType.LIKE);
	...
	public void findByFilters() {
		...
		List<User> users = service.find(filters);
		assertEquals(1, users.size());
		assertEquals("admin", users.get(0).getLoginName());
		...
		users = service.find(filters);
		assertEquals(6, users.size());
		assertTrue(StringUtils.contains(users.get(0).getEmail(), "springside.org.cn"));

		//Filter with Page
		PageRequest pageRequest = new PageRequest(1, 5);
		Page<User> page = service.findPage(pageRequest, filters);
		assertEquals(5, page.getResult().size());
		assertEquals(6L, page.getTotalItems());

		pageRequest.setPageNo(2);
		page = service.findPage(pageRequest, filters);
		assertEquals(1, page.getResult().size());
	...
	public void findPageByHqlAutoCount() {
		PageRequest pageRequest = new PageRequest(1, 5);
		Page<User> page = service.findPage(pageRequest, "from User user");
		assertEquals(6L, page.getTotalItems());

		page = service.findPage(pageRequest, "select user from User user");
		assertEquals(6L, page.getTotalItems());

		page = service.findPage(pageRequest, "select user from User user order by id");
		assertEquals(6L, page.getTotalItems());

		page = service.findPage(pageRequest, "select user from User user where user.id=1 order by id");
